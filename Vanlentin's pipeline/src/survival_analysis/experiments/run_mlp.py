#!/usr/bin/env python3
"""
Train and evaluate MLPModule across all splits, reporting mean C-index and IBS.

Usage:
    cd "Vanlentin's pipeline"
    python -m survival_analysis.experiments.run_mlp
"""
...
def run_single_split(split_index: int, n_splits: int) -> tuple[float, float, float]:
    """Train and evaluate MLPModule on one split; return (C-index, IBS, C-index std)."""
    print(f"\n{'='*55}")
...
def main() -> None:
    with open(SPLITS_JSON) as f:
        splits = json.load(f)
    n_splits = len(splits)
    print(f"\nStarting MLP experiment over {n_splits} splits\n")
    ...
    print(f"\n{'='*55}")
    print("  MLP — Summary across all splits")
    print(f"{'='*55}")
    ...
    output_path = REPO_ROOT / "results_mlp_all_splits.csv"
